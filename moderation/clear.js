...
if(isNaN(amount) || !Number.isInteger(amount)) return message.channel.send("Ingrese una cantidad de mensajes para borrar.");
if(!amount || amount < 2 || amount > 100) return message.channel.send("Por favor introduzca un número de mensaje entre 2 y 100.")
if(!args[1]) {
...
await message.channel.bulkDelete(amount).then(async (m) => { 
  ...
  .setDescription(`✅  Cleared **${m.size}**/**${amount}** messages!`);
   ...
   message.channel.send(embed).then(msg => msg.delete({timeout:4000})); 
     ...
     console.log(e) 
     message.channel.send(`Sólo puede eliminar los mensajes que no tengan más de 14 días.`)
...
break;
 ...
 return message.channel.send(`Ocurrió un error.`)
  ...
  console.log(error)
  message.channel.send(`Ocurrió un error: \`${error}\``)
}
